...
- **覆盖率门禁**（`pnpm run test:coverage`）：完整的本地运行会对 `packages/*/*/src` 执行按文件 100% 覆盖；PR CI 对源代码或测试有改动的包保持该阈值，两者都未改动时跳过覆盖率。仅依赖项的 PR 会改跑无密钥单元测试。未覆盖的行往往是门禁正确标记出的死代码（应删除），而非需要补写的测试。行覆盖率是必要条件，但永远不是充分条件：它证明行被执行过，不证明功能按交付预期工作。`packages/shell/pwsh-local/src` 的按文件 100% 覆盖需要真实的 `pwsh`：缺少它时其执行器套件会自动跳过，`vitest.config.ts` 会豁免该文件以使无 pwsh 的主机保持绿色，而 CI runner 自带 pwsh，仍按完整标准执行门禁。
