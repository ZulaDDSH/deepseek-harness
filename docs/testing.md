...
How this repository tests and keeps results meaningful. Commands live in root [AGENTS.md](../AGENTS.md); linked Agent Notes carry the rationale.
...
- **Coverage gate** (`pnpm run test:coverage`): the full local run enforces per-file 100% on `packages/*/*/src`; pull-request CI keeps that threshold for packages whose source or tests changed and skips coverage when neither changed. Dependency-only pull requests run the keyless unit suite instead. Delete genuinely dead lines rather than adding tests only to satisfy coverage. Line coverage is necessary, never sufficient — it proves lines ran, not that the feature works as shipped. Per-file 100% on `packages/shell/pwsh-local/src` needs a real `pwsh`: without one its executor suites self-skip and `vitest.config.ts` exempts the file so pwsh-less hosts stay green, while CI runners ship pwsh and enforce the full bar.
