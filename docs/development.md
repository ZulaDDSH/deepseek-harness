...
Every pull request runs frozen-lockfile installation, static dependency, package, and policy checks, typecheck, lint, build and package-artifact validation, Python SDK/runtime validation, Node 22.19/24.9/26 compatibility, and native Windows build/tests. Changes to package source or tests run the unit suite with the unchanged 100% per-file threshold scoped to packages whose `src/` or `tests/` changed. Recorded-session and Web snapshots run for product-code changes; dependency-manifest and lockfile-only updates skip changed-package coverage and those broad consumer suites. These gates use GitHub-hosted Ubuntu and Windows runners and require no API, App, Cloudflare, or publishing credentials.

After a master push, the reusable Python runtime checks add Linux ARM64 and macOS ARM64/x64. The Node-addon workflow runs its native OS/architecture matrix only when its paths change. Sandbox confinement proofs run on supported Linux and macOS kernels. Performance benchmarks are manual and observational. Package packing stays on PRs; npm/PyPI publication and Pages deployment are separate manual release operations. See [scripts/run-gates.ts](../scripts/run-gates.ts) and [the workflow directory](../.github/workflows) for gate definitions.
