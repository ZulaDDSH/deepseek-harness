...
默认情况下，服务器指令会为每个已配置的 MCP 服务器贡献一个作用域内的字面系统提示词段落。当某台服务器的指导并非必需时，客户端可以设置 `includeServerInstructions: false`；连接、工具、资源和调用方可见服务器名称仍保持可用，但会省略指令段落。已有的系统消息日志只记录实际送达模型的组装后指令。资源内容保持按需读取，因此连接服务器不会将其文档预加载到提示词中。
...
仅提供资源的服务器无需添加按服务器区分的模型工具即可使用。调用方可见的服务器名称始终产生共享提示词成本；启用的服务器指令会增加提示词 token，省略的指令则不会。资源文档仅在读取时增加 token。只要调用方可见的客户端仍已配置，连接失败期间共享 schema 就保持稳定，但所选服务器不可用时调用仍会失败。二进制资源仍是程序化值，分页遵循 SDK。
