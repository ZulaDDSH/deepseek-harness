...
A Remote method takes JSON arguments and returns a value; it cannot take a callback, and a running call has no reply path back into it. The host-to-browser direction that *can* carry an answer is a forwarded `waterfall` event, and that mechanism is Agent-scoped — it requires the event to carry the Agent whose Context receives it, and validates the pair before delivering. An authorization attempt has no Agent: a configuration page starts it. So the conversation is split across the two directions that do exist without Agent scope.

`AuthorizationController` in `dsh-api-settings-controller` owns that split. `begin` is one long-running call that stays pending for as long as the human takes, publishing each notice as the forwardable `authorization/notice` event addressed to a Host-minted attempt id; the human's answers come back as ordinary `answer` calls addressed by that same id. `authorization/notice` is declared alongside the flow about to run and carries the question itself, so a surface that lost a notice cannot answer a prompt it never showed. A withdrawn attempt settles as `cancelled`, a failed one travels as a rejection, and both are distinguished from a declined question, which is an outcome.

Two consequences follow from that shape rather than from a timeout. An attempt is not resumable across a page reload — the attempt id dies with the Host-side entry, which is why the recorded limit above still stands. And on the browsing side, `ui-settings-models` reads `remote.authorization` optionally: a deployment that mounts no registry keeps its API-key fields and offers no sign-in, so the surface never assumes the seam.
...
The wire half is pinned by `authorization-controller.host.spec.ts` against a real registry, a real credential store, and a flow registered the way an adapter registers one: the namespace and its method set, the entry list joined with stored-credential state, notice publication addressed to the attempt, a question parking until its `answer` call settles it and the flow finishing with what the human typed, a `select` question carrying its options, a second answer to one question refusing rather than resolving twice, a withdrawn attempt settling `cancelled`, a flow failure mapping to `authorization/failed`, and both an unregistered key and a key outside the record grammar refusing by name. The Codex catalog gate is pinned in `catalog.spec.ts`: withheld ids absent, a withheld id still served when a profile names it, and a provider sharing that id over the public API untouched.
