...
### The surface, and the wire constraint that shaped it

A Remote method takes JSON arguments and returns a value; it cannot take a callback, and a running call has no reply path back into it. The host-to-browser direction that *can* carry an answer is a forwarded `waterfall` event, and that mechanism is Agent-scoped — it requires the event to carry the Agent whose Context receives it, and validates the pair before delivering. An authorization attempt has no Agent: a configuration page starts it.

`AuthorizationController` in `dsh-api-settings-controller` owns the conversation through the two directions that exist without Agent scope, over a stream rather than a forwarded event. `begin` is a `mode: 'stream'` method: its first item names an unguessable per-attempt capability, every later item is a notice, and the last names how the attempt ended. The human's answers come back as ordinary `answer` calls addressed by that capability.

**Why not a forwarded `emit` event.** A notice carries an authorization URL, a device code, or a question, and `broadcastRemoteEvent` delivers an `emit`-mode frame to *every* connected client — the `waterfall` path is scoped by its Agent id, while `emit` has no equivalent. Publishing notices that way handed one browser tab's OAuth URL and device code to every other open tab, and let any tab answer or cancel an attempt it never started. Stream results are written to the socket the calling client opened, so the carrier itself is the scoping mechanism; the capability is minted from `randomBytes` rather than derived from the key or a counter, so it can be neither guessed nor enumerated. A withdrawn attempt settles as `cancelled`, a failed one rejects the stream, and a declined question is an outcome rather than a breakage.

Two consequences follow from that shape rather than from a timeout. An attempt is not resumable across a page reload — the capability dies with the Host-side entry, which is why the recorded limit above still stands. And on the browsing side, `ui-settings-models` reads `remote.authorization` optionally: a deployment that mounts no registry keeps its API-key fields and offers no sign-in, so the surface never assumes the seam.

The catalog is narrower than pi-ai's for the same reason the surface exists. pi-ai's `openai-codex` catalog is transcribed from models.dev, which lists what the platform serves rather than what one plan may use, and the backend refuses `gpt-5.3-codex-spark`, `gpt-5.4`, and `gpt-5.4-mini` for a ChatGPT subscription. `catalogModels` withholds exactly those ids for that route, so a default route offers only models that answer; a profile naming one explicitly in its `models` list still receives it, because the configuration the user wrote is not silently dropped.
...
`openai-codex` returns to the provider picker and to the Models page directory. All 38 installed providers offer sign-in: 31 collect a key through pi-ai's own prompt, six offer that beside a subscription login, and Codex offers only the subscription login. The `authorization` registry is mounted by the base bundle, so every base-backed profile reaches the flows rather than only an in-process caller.

The surface is the Models-page sign-in control, described above. A login now runs from the page: the card renders the flow's notices, opens the page or shows the code the flow reported, and returns a typed answer for each question.
...
The wire half is pinned by `authorization-controller.host.spec.ts` against a real registry, a real credential store, and a flow registered the way an adapter registers one: the namespace and its method set, `begin` carrying stream mode, the entry list joined with stored-credential state, notices delivered on the attempt's own stream and stamped with its capability, capabilities that are long, random, and distinct per attempt, a question parking until its `answer` call settles it and the flow finishing with what the human typed, a `select` question carrying its options, a second answer to one question refusing rather than resolving twice, a question whose own signal is already aborted settling the flow instead of hanging, a withdrawn attempt settling `cancelled`, a flow failure rejecting the stream, and both an unregistered key and a key outside the record grammar refusing by name.

Isolation is its own suite there, because a notice carries an authorization URL and a device code: two callers driving their own attempts each receive only their own notices, and an answer or cancel addressed by a capability the caller never received is refused as `authorization/not-found` while the real attempt stays open and answerable by its owner. Those three cases fail against the broadcast implementation this replaced. `signin-card.client.spec.tsx` pins the surface: a notice's page and code render, a refused answer stays on screen with the Host reason, an accepted answer clears the question, a completed sign-in reports so the page refreshes, and a withdrawn attempt reports nothing.

The Codex catalog gate is pinned in `catalog.spec.ts`: withheld ids absent, a withheld id still served when a profile names it, and a provider sharing that id over the public API untouched.
