...
async function deepseekDefaultsServer(options: { waitForTitleRequest?: boolean; protocol?: 'messages'; keepAliveCount?: number } = {}): Promise<DeepSeekDefaultsServer> {
...
describe('headless stream-json snapshots', () => {
  ...
  it('keeps provider comments alive and sends DeepSeek defaults through the one-shot app', async () => {
    const server = await deepseekDefaultsServer({ keepAliveCount: 25, protocol: 'messages' })
    try {
      const result = await runLoaderSmoke({
