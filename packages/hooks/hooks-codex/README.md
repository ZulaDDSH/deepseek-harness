...
- **`PreToolUse` is partial** — blocking works, but `additionalContext`, `permissionDecision: "allow"`, and `updatedInput` are ignored. Shell calls use `tool_input: { command }`; DSH file tools expose their parsed arguments, which is broader than Codex's shell-only payload.
- **`PostToolUse` is partial** — blocking feedback and JSON `additionalContext` work, but `{"continue": false}` is not enforced, shell arguments use `{ command }`, DSH file-tool arguments are retained, and structured tool output is flattened to text in `tool_response`.
