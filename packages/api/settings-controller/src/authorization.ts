...
export class AuthorizationController extends TypertRemoteService {
  ...
  async *begin(
    ...
    // A queue bridges the flow's synchronous callbacks to this generator, so a
    // notice is delivered in order even when the flow emits several in a row.
    ...
    return {
      ...
      prompt: (prompt) => {
        ...
        return new Promise<string>((resolve, reject) => {
          // A question answered before its listener is attached must settle the
          // attempt rather than run the flow on a withdrawn question.
          ...
          // Registered before the notice is queued, so an answer that arrives
          // on the next tick finds the question it belongs to.
          ...
          // declining, so it rejects with a plain error. A signal already
          // aborted has no event left to fire, so it is checked directly.
          if (prompt.signal?.aborted === true) {
            onWithdrawn()
            return
          }
          // The listener is detached whenever the question settles, so a long
          // attempt does not accumulate one listener per question asked.
