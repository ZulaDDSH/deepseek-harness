...
/**
 * Normalize the OpenCode Go usage payload into quota windows, keeping any
 * window that reports at least one of a percentage, a reset instant, or a
 * provider status: a status alone still marks a window that cannot serve.
 * @param payload - decoded OpenCode Go usage response.
 * @returns the recognized windows, keyed by normalized window id.
 */
