...
| `proactiveToolResultPruning` | `false` | When the tool-result pruner is mounted, trim oversized tool results before the pressure threshold check instead of waiting for compaction pressure. |
...
Mount `dsh-compaction-tool-result-pruner` before this package to trim oversized tool results. Trimming makes no model call and can remove the need to summarize at all: when the trimmed conversation fits within the threshold, condensation skips the summary. By default the cache-first behavior waits until pressure qualifies; set `proactiveToolResultPruning: true` to prune before the threshold check on pressure passes. Proactive pruning lowers repeated input sooner but rewrites older history, so provider prefix-cache reuse is invalidated from the first changed result.
