...
| `proactiveToolResultPruning` | `false` | When the tool-result pruner is mounted, trim oversized results from completed earlier turns before the pressure threshold check; current-turn results remain verbatim. |
...
Mount `dsh-compaction-tool-result-pruner` before this package to trim oversized tool results. Trimming makes no model call and can remove the need to summarize at all: when the trimmed conversation fits within the threshold, condensation skips the summary. By default the cache-first behavior waits until pressure qualifies. Set `proactiveToolResultPruning: true` to prune oversized results from completed earlier turns before the threshold check on automatic pre-step pressure passes. Results produced in the current turn stay verbatim so the model receives fresh tool evidence at least once. Proactive pruning lowers repeated input sooner but rewrites older history, so provider prefix-cache reuse is invalidated from the first changed result.
