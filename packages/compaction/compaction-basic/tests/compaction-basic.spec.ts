...
describe('compact configuration and defaults', () => {
  ...
  it('merges exact provider/model policy overrides and scales ratios per model', () => {
    ...
    const capped = resolveTargetPolicy(resolveConfig({ headroomTokens: 0, maxTokens: 8192, maxContextWindow: 200_000 }), {
      provider: 'capped-provider', model: 'capped-model',
    })
    expect(resolveCompactSpec(capped, 1_000_000, 0)).toMatchObject({
