---
description: "dsh Web 客户端的共享 Workspace 浏览器与选择器插件：分组或扁平的会话行、Workspace 颜色与图标、添加、重命名、重排序、搜索、fork、归档，以及目录流选取子 slot。"
...
本包让用户浏览分组或扁平的 Session 列表、为每个 Workspace 指定颜色与图标、为新 Session 选择 Workspace，并通过添加、重命名、重排序、搜索、fork、归档和删除 Workspace 来管理 Workspace 与 Session。待处理交互显示为警告点，活动定时任务显示为闹钟标识，subagent 来源的 Session 则保持隐藏。规范化后仍有差异的文件夹路径会保留为独立 Workspace。添加 Workspace 需要组合目录选择器；没有目录选择器时，添加操作不可用。
...
每个 Workspace 都带有一个颜色和一个图标，可通过其行菜单的**颜色**和**图标**子菜单、在该行上右键，或同一菜单中的**自定义外观**来选择。颜色会绘制到该 Workspace 的文件夹图标及其标题上；图标则替换该图标。任一子菜单中的**默认**都会清除该部分并恢复普通外观。

**颜色**与**图标**两行同时也是筛选器：区头的筛选控件可把列表收窄到一种颜色、一个图标或两者兼有，**全部**则分别清除每一部分。筛选仅作用于 Workspace 分组，因此在每种分组方式下都可用。

选择结果保存在浏览器本地，并在重新加载后保留。Session 与 Chat 分组通过同一套词汇解析，因此 Session 行可以在标题前显示所属 Workspace 的图标，并继承其颜色。
...
点击标题栏的**新建分组**可创建命名分组，它以工作区列表下方的面板呈现。分组是工作区列表之上的已保存视觉筛选，而不是第二份成员关系：归类一个对话只是把它加入该面板，并不从工作区文件夹中移除任何东西，因此同一个对话在两处都可见。只要存在任何分组（空分组也算），该面板就会显示，而工作区列表在其中保留完整的控件集合和每一种**分组方式**模式。分组不承载工作区指令、环境、文件或共享上下文语义，一个 Session 至多属于一个分组。

可以从对话行菜单的**移动到分组 → [名称]**归类，或把对话拖到分组标题、或该分组块内的任意位置。拖到同级对话上则插入到该位置，把一个分组标题拖到另一个上则重排分组。**从分组中移出**会把对话送回未分组列表。分组标题的菜单提供重命名、删除，以及与外观控件共享的颜色和图标选项。删除分组只删除分组关系：其中的对话保留日志和 Workspace 归属，并重新出现在未分组列表中。

分组、折叠状态、成员分配和保存的顺序都是浏览器本地的，并跨重载持久化。它们不会随 Session 迁移到其他浏览器、机器或配置，也不改变 Session 的工作区、工作目录、日志、归档状态或搜索参与。
...
标题宽于所在行时始终以省略号裁切，因此指针移动不会移动行内容。已有的悬浮卡片会展示完整标题和复制操作。
...
`appearance.ts` 持有词汇表——五种颜色与八个图标——以及用于校验解码值的两个类型守卫。颜色使用字面十六进制而非主题别名，因为色块必须显示该行实际绘制的颜色；同一映射同时用于行、菜单色块和对话框选项。`rows/WorkspaceIcons.ts` 通过一份全量记录把每个图标选项绑定到其字形，因此新增选项而缺少字形会导致编译失败。

`WorkspaceBrowser` 把三份 id 到外观的映射保存为本地状态，并在变化时写入 `localStorage`；Session 的选择与其 Workspace 的选择相互独立，因此重绘 Workspace 不会重绘其 Session。`updateAppearance` 合并单次变化，并在两部分都被清除后删除该条目，因此存储值不会持续增长。持久化键存放的是合并后的值；此前单映射键写入的值仍会被读取，因此已有选择在升级后依然保留。
