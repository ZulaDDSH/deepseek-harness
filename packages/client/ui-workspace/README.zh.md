---
description: "dsh Web 客户端的共享 Workspace 浏览器与选择器插件：Workspace、单列表与活动会话视图、添加、重命名、重排序、搜索、fork、归档，以及目录流选取子 slot。"
...
本包让用户浏览分组、单列表或以活动为中心的 Session 列表、为新 Session 选择 Workspace，并通过添加、重命名、重排序、搜索、fork、归档和删除 Workspace 来管理 Workspace 与 Session。待处理交互和实时运行状态会在状态点旁显示可见文字，活动定时任务显示为闹钟标识，subagent 来源的 Session 则保持隐藏。规范化后仍有差异的文件夹路径会保留为独立 Workspace。添加 Workspace 需要组合目录选择器；没有目录选择器时，添加操作不可用。
...
标题宽于所在行时会持续以省略号裁切，因此指针移动不会带动行内容。现有悬浮卡片会显示完整标题并提供复制操作。

### 待处理交互

Session 行渲染运行时的实时 `pendingInteraction` 分类：审批显示**等待审批**，计划审阅显示**计划待审**，普通问题显示**等待回答**。主要的待处理或运行状态会在状态点旁以紧凑文字直接显示，因此不依赖颜色辨识或悬浮。待处理交互优先于运行活动。
