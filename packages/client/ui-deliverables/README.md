...
Two surfaces read the current status of a registered Workspace through the Host: the left Sidebar's Changes panel, which opens in the main area, and the right Sidebar's Changes page, reached from that panel's own tab picker. Both list tracked modifications, additions, deletions, renames, and untracked files with Git status markers and line counts, and show the selected file's current comparison against HEAD. The right-column page resolves the Workspace whose path matches the current Session's working directory and falls back to the first registered one, offering a picker only when more than one exists; its narrow column stacks the file list above the comparison.
